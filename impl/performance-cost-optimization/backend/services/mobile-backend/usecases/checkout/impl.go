...
// mapOrderToResponse converts a domain Order to a slim OrderResponse.
func mapOrderToResponse(o *order.Order) *OrderResponse {
...
func (uc *checkoutUseCase) PlaceOrder(ctx context.Context, req PlaceOrderRequest) (*OrderResponse, error) {
	...
	return mapOrderToResponse(o), nil
}
...
func (uc *checkoutUseCase) GetOrderHistory(ctx context.Context, req GetOrderHistoryRequest) (*OrderHistoryResponse, error) {
	...
	slimOrders := make([]OrderResponse, 0, len(orders))
	for _, o := range orders {
		slimOrders = append(slimOrders, *mapOrderToResponse(&o))
	}
